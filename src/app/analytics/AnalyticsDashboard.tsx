...
export default function AnalyticsDashboard() {
    ...
    const handleDownloadReceipt = async (booking: { id: string; confirmationId: string; date: string; time: string; status: string; venue: { name: string; category: string; address?: string } }) => {
        ...
        try {
            ...
            const safe = (t: string | undefined | null) => (t || "").replace(/[^\x20-\x7E]/g, "?");
            const text = (t: string, opts: object) => { try { page.drawText(t, opts); } catch { /* skip */ } };

            // Header bar
            page.drawRectangle({ x: 0, y: height - 10, width, height: 10, color: rgb(0.23, 0.51, 0.96) });
            y -= 60;
            text("WORKSPHERE CONFIRMATION", { x: 150, y, size: 24, font: boldFont, color: rgb(0, 0, 0) });
            y -= 15;
            text("SECURE NEURAL TRANSACTION RECEIPT", { x: 180, y, size: 8, font, color: rgb(0.5, 0.5, 0.5) });
            y -= 50;

            // Details
            text("BOOKING DETAILS:", { x: 50, y, size: 12, font: boldFont, color: rgb(0, 0, 0) });
            y -= 15;
            text("-".repeat(50), { x: 50, y, size: 10, font, color: rgb(0, 0, 0) });
            y -= 20;
            text(`REFERENCE ID: ${safe(booking.confirmationId)}`, { x: 50, y, size: 10, font, color: rgb(0, 0, 0) }); y -= 18;
            text(`VENUE: ${safe(booking.venue.name)}`, { x: 50, y, size: 10, font, color: rgb(0, 0, 0) }); y -= 18;
            text(`CATEGORY: ${safe(booking.venue.category?.toUpperCase())}`, { x: 50, y, size: 10, font, color: rgb(0, 0, 0) }); y -= 18;
            text(`ADDRESS: ${safe(booking.venue.address || "Verified Workspace")}`, { x: 50, y, size: 10, font, color: rgb(0, 0, 0) }); y -= 18;
            text(`SCHEDULE: ${safe(booking.date)} @ ${safe(booking.time)}`, { x: 50, y, size: 10, font, color: rgb(0, 0, 0) }); y -= 18;
            text(`STATUS: ${safe(booking.status)}`, { x: 50, y, size: 10, font, color: rgb(0, 0, 0) }); y -= 40;

            text("SECURITY PROTOCOL:", { x: 50, y, size: 12, font: boldFont, color: rgb(0, 0, 0) }); y -= 18;
            text("ZERO-FEE ACCESS PROTOCOL ACTIVE", { x: 50, y, size: 10, font, color: rgb(0, 0, 0) }); y -= 18;
            text("ENCRYPTED VIA WORKSPHERE L3", { x: 50, y, size: 10, font, color: rgb(0, 0, 0) }); y -= 80;
            text("Thank you for choosing WorkSphere. Your workspace is ready for you.", { x: 80, y, size: 8, font, color: rgb(0.4, 0.4, 0.4) });
        ...
        return (
            <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 font-sans flex items-center justify-center p-6">
                <div className="bg-red-500/10 border border-red-500/25 p-8 rounded-[2rem] max-w-md w-full text-center shadow-2xl">
                    <h2 className="text-lg font-black text-red-500 uppercase tracking-wider mb-2">Neural Link Error</h2>
                    <p className="text-xs text-zinc-500 dark:text-zinc-400 mb-6 font-bold leading-relaxed">{(data as any).error}</p>
                    <button
                    ...
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-[2.5rem] shadow-sm">
                            ...
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <span className="text-[10px] text-zinc-400 font-mono">
                                        XP: {data?.gamification?.xp} / {(data?.gamification?.xp || 0) - (data?.gamification?.xpInCurrentLevel || 0) + (data?.gamification?.xpForNextLevel || 0)}
                                    </span>
                                </div>
                        ...
                        <div className="relative w-full h-4 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden p-0.5">
                            <div
                                ...
                                style={{ width: `${data?.gamification?.progressPercent || 0}%` }}
                            />
                        </div>
                        <div className="flex justify-between items-center mt-3 text-[10px] font-black uppercase tracking-widest text-zinc-400">
                            ...
                            <span>{(data?.gamification?.xpForNextLevel || 0) - (data?.gamification?.xpInCurrentLevel || 0)} XP to Level {(data?.gamification?.level || 0) + 1}</span>
                                    ...
                                    <div className="flex items-center gap-5">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-wider ${booking.status === 'CONFIRMED' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' :
                            ...
                            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-[2.5rem] shadow-sm space-y-6">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-[10px] font-mono text-zinc-400">
                                        {data?.gamification?.badges?.filter(b => b.earned).length || 0} / {data?.gamification?.badges?.length || 0} Earned
                                    </span>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    {data?.gamification?.badges?.map((badge) => (
                                        <div key={badge.id} className="group relative flex flex-col items-center">
                                            ...
                                            <div className={`relative w-16 h-16 rounded-2xl flex items-center justify-center p-0.5 transition-all duration-300 group-hover:scale-105 ${badge.earned
                                            ...
                                            <div className="absolute bottom-20 scale-0 group-hover:scale-100 transition-all origin-bottom duration-200 bg-zinc-950 text-white text-[10px] p-4 rounded-xl shadow-2xl border border-zinc-800 w-44 z-30 pointer-events-none opacity-0 group-hover:opacity-100">
                                                <div className="font-black uppercase tracking-wider text-[9px] mb-1 flex items-center justify-between">
                                                    ...
                                                    <span className={`text-[7px] px-1.5 py-0.5 rounded font-mono ${badge.earned ? 'bg-green-500/20 text-green-400' : 'bg-zinc-800 text-zinc-400'
                                                        }`}>
