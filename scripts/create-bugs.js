...
if (!token || token === 'github_pat_antigravitydummytoken') {
  ...
  console.error('  node scripts/create-bugs.js');
  console.error('\x1b[36m%s\x1b[0m', 'macOS / Linux:');
  console.error('  GITHUB_TOKEN="your_personal_access_token" node scripts/create-bugs.js');
  process.exit(1);
}

const issuesPath = path.join(__dirname, 'bug-data.json');
if (!fs.existsSync(issuesPath)) {
  console.error('\x1b[31m%s\x1b[0m', '❌ Error: bug-data.json not found in scripts directory.');
  process.exit(1);
}
...
console.log('\x1b[35m%s\x1b[0m', `🚀 Starting creation of ${issues.length} bug issues in repository ${repo}...`);
...
async function run() {
  ...
  console.log('\x1b[32m%s\x1b[0m', `🎉 Process finished. Created ${successCount}/${issues.length} bugs successfully.`);
  console.log('\x1b[35m%s\x1b[0m', '=============================================');
}
