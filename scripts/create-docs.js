...
if (!token || token === 'github_pat_antigravitydummytoken') {
  ...
  console.error('  node scripts/create-docs.js');
  console.error('\x1b[36m%s\x1b[0m', 'macOS / Linux:');
  console.error('  GITHUB_TOKEN="your_personal_access_token" node scripts/create-docs.js');
  process.exit(1);
}

const issuesPath = path.join(__dirname, 'docs-data.json');
if (!fs.existsSync(issuesPath)) {
  console.error('\x1b[31m%s\x1b[0m', '❌ Error: docs-data.json not found in scripts directory.');
  process.exit(1);
}
...
console.log('\x1b[35m%s\x1b[0m', `🚀 Starting creation of ${issues.length} documentation issues in repository ${repo}...`);
...
async function run() {
  ...
  console.log('\x1b[32m%s\x1b[0m', `🎉 Process finished. Created ${successCount}/${issues.length} docs issues successfully.`);
  console.log('\x1b[35m%s\x1b[0m', '=============================================');
}
