...
if (!token || token === 'github_pat_antigravitydummytoken') {
  ...
  console.error('  node scripts/create-all-new-issues.js');
  console.error('\x1b[36m%s\x1b[0m', 'macOS / Linux:');
  console.error('  GITHUB_TOKEN="your_personal_access_token" node scripts/create-all-new-issues.js');
  process.exit(1);
}

const issuesPath = path.join(__dirname, 'new-issues-data.json');
if (!fs.existsSync(issuesPath)) {
  console.error('\x1b[31m%s\x1b[0m', '❌ Error: new-issues-data.json not found in scripts directory.');
  process.exit(1);
}
...
console.log('\x1b[35m%s\x1b[0m', `🚀 Starting creation of ${issues.length} issues in repository ${repo}...`);
...
async function run() {
  ...
  for (let i = 0; i < issues.length; i++) {
    ...
    console.log(`[${i + 1}/${issues.length}] Creating [${issue.type}]: "${issue.title}"...`);
    try {
